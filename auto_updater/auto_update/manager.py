...
class AutoUpdateManager:
    ...
    def _check_and_update_process(self, process: ManagedProcess) -> None:
        ...
        try:
            ...
            # Perform update
            success = self.orchestrator.update_neuron_process(
                process.neuron_type, 
            ...
            try:
                ...
                if current_time - self.last_check_time >= self.config.check_interval_seconds:
                    ...
                    if self._is_in_failure_cooldown():
                        remaining_cooldown = self.config.failure_cooldown_seconds - (current_time - self.last_failure_time)
                        self.logger.warning(f"In failure cooldown, skipping update check. {remaining_cooldown:.0f}s remaining")
                    else:
