...
static std::string run_device(int dev, uint64_t challenge_seed, int iters, double fill_ratio, int d, int matrices) {
    auto t0 = std::chrono::steady_clock::now();
    uint64_t seed = challenge_seed;  // one card, one seed (gt_attest added the device index; the bank is per seed)
    ...
    // fill: fill_ratio of the card's TOTAL VRAM (capped at what is free minus headroom), in large chunks; the
    // working set is the first chunk. fill_ratio 0 = the working set only.
    std::vector<void*> chunks; size_t filled = 0;
    size_t want = (size_t)(fill_ratio * (double)total_b); size_t cap = free_b - (256u << 20);
    if (want > cap) want = cap; if (want < work_bytes) want = work_bytes;
    while (filled < want) {
    ...
    char driver[NVML_SYSTEM_DRIVER_VERSION_BUFFER_SIZE] = ""; nvmlSystemGetDriverVersion(driver, sizeof driver);
    char out[1280];
    std::snprintf(out, sizeof out, "{\"seed\":%llu,\"device\":%d,\"uuid\":\"%s\",\"name\":\"%s\",\"driver\":\"%s\",\"sm_count\":%d,\"vram_total\":%zu,\"vram_free_before\":%zu,\"filled_bytes\":%zu,\"fill_ratio\":%.4f,\"dim\":%d,\"matrices\":%d,\"iters\":%d,\"digest\":\"%s\",\"wall_ms\":%.1f}",
        (unsigned long long)seed, dev, device_uuid(dev).c_str(), prop.name, driver, prop.multiProcessorCount, total_b, free_b, filled, fill_ratio, d, matrices, iters, sha.hex().c_str(), ms);
    return std::string(out);
}

int main(int argc, char** argv) {
    uint64_t seed = 0; int iters = 3, d = 1024, matrices = 512, dev = 0; double fill_ratio = 0.9; bool have_seed = false;
    for (int i = 1; i < argc; i++) {
        ...
        else if (a == "--device") dev = std::atoi(next());
        else if (a == "--fill-ratio") fill_ratio = std::atof(next());
        else { std::fprintf(stderr, "unknown arg %s\n", a.c_str()); return 2; }
    }
    if (!have_seed || iters < 1 || d % TILE != 0 || d < TILE || matrices < 2 || fill_ratio < 0.0 || fill_ratio > 1.0) {
        std::fprintf(stderr, "usage: gt_challenge --seed <u64> [--fill-ratio 0..1] [--iters n] [--device i] [--dim d] [--matrices m]\n"); return 2;
    }
    nvmlInit_v2();
    int count = 0; CHECK(cudaGetDeviceCount(&count));
    if (dev < 0 || dev >= count) { std::fprintf(stderr, "no device %d (count %d)\n", dev, count); return 2; }
    std::printf("%s\n", run_device(dev, seed, iters, fill_ratio, d, matrices).c_str());
    nvmlShutdown();
    return 0;
}
