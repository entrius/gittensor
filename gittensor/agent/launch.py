...
def down_commands(
    ...
    so nothing keeps serving with no agent behind it and no orphan holds a workload port for the next `gitt up`). The
    agent before the workloads: with it gone the controller sees only "unreachable", never a container gone under a
    live agent (which is a bench)."""
    return [
