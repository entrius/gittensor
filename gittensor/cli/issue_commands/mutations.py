...
def issue_register(
    ...
    if not yes:
        if not sys.stdin.isatty():
            raise click.ClickException('Non-interactive mode detected. Use --yes / -y to skip confirmation.')
    ...
    try:
        ...
        if not contract_metadata.exists():
            raise click.ClickException(f'Contract metadata not found at {contract_metadata}')
...
def issue_harvest(wallet_name: str, wallet_hotkey: str, network: str, rpc_url: str, contract: str, verbose: bool):
    ...
    if not contract_addr:
        raise click.ClickException('Contract address not configured. Set CONTRACT_ADDRESS env var or run ./up.sh --issues.')
