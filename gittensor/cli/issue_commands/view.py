...
def _fill_percent(bounty: int, target: int) -> float:
    """Calculate the fill percentage of a bounty against a target.

    Ensures consistent calculation using Decimal to avoid floating point artifacts.
    """
    if not target or target <= 0:
        return 0.0
    return float((Decimal(bounty) / Decimal(target) * Decimal(100)).quantize(Decimal('0.000001')))
