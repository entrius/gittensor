...
# Hardcoded emission splits: unified scoring pool (PR + issue discovery) + treasury.
# Recycle (UID 0) receives only slack — unclaimed per-repo OSS slices and
# ``(1 - sum(emission_share)) * OSS_EMISSION_SHARE`` when registry shares sum below 1.0.
OSS_EMISSION_SHARE = 0.90  # combined scoring pool (repo slices + within-repo PR/issue split)
