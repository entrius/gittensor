...
async def reward(
    ...
    valid_prs, open_pr_count, closed_pr_count = get_user_merged_prs_graphql(
        miner_eval.github_id, miner_eval.github_pat, master_repositories
    )
    
    miner_eval.total_merged_prs = len(valid_prs)
    miner_eval.total_open_prs = open_pr_count
    miner_eval.total_closed_prs = closed_pr_count

    for raw_pr in valid_prs:
