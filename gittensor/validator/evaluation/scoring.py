...
def score_pull_requests(
    ...
    for n, pr in enumerate(miner_eval.pull_requests, start=1):
        ...
        if (
            (pr.merged_at and pr.merged_at > MERGE_SUCCESS_RATIO_APPLICATION_DATE) or
            (pr.closed_at and pr.closed_at > MERGE_SUCCESS_RATIO_APPLICATION_DATE)
        ):
            merge_success_multiplier = calculate_merge_success_multiplier(miner_eval)
        else:
            merge_success_multiplier = 1.0
