...
def check_issue_active(validator: 'Validator', issue_id: int) -> tuple[str | None, ContractIssue | None]:
    """Verify issue is in a predictable state on-chain. Returns (error, issue)."""
    try:
        from gittensor.validator.utils.issue_competitions import get_contract_address

        contract_addr = get_contract_address()
        if not contract_addr:
            return 'Issue bounties not configured on this validator', None
        ...
        if issue is None:
            return f'Issue {issue_id} not found on-chain', None
        if issue.status not in (IssueStatus.REGISTERED, IssueStatus.ACTIVE):
            return f'Issue {issue_id} is not in a predictable state (status: {issue.status.name})', None
    except Exception as e:
        bt.logging.warning(f'Failed to check issue state for {issue_id}: {e}')
        return f'Could not verify issue state: {e}', None
...
def check_prs_open(repository: str, issue_number: int, predictions: dict[int, float]) -> str | None:
    ...
    try:
        ...
        open_prs = find_prs_for_issue(repository, issue_number, open_only=True, token=GITTENSOR_VALIDATOR_PAT)
