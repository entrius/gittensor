...
_DEFAULT_ISSUE_DISCOVERY_SHARE = 0.5


class RepositoryRegistryError(ValueError):
    """Raised when repository emission-share config violates the registry contract."""
...
class RepositoryConfig:
    ...
    def __init__(
        ...
        *,
        ...
        issue_discovery_share: float = _DEFAULT_ISSUE_DISCOVERY_SHARE,
        inactive_at: Optional[str] = None,
        additional_acceptable_branches: Optional[List[str]] = None,
        trusted_label_pipeline: bool = False,
        label_multipliers: Optional[Dict[str, float]] = None,
...
def _coerce_share(repo_name: str, field_name: str, raw_value: Any) -> float:
    if isinstance(raw_value, bool):
        raise RepositoryRegistryError(f'{repo_name} {field_name} must be a finite float, got bool')
    value = float(raw_value)
    if not isfinite(value):
        raise RepositoryRegistryError(f'{repo_name} {field_name} must be a finite float, got {raw_value!r}')
    return value
...
def load_master_repo_weights() -> Dict[str, RepositoryConfig]:
    ...
    Returns:
        ...
        ValueError when repository entries violate the emission-share contract.
    """
            ...
            try:
                ...
                config = RepositoryConfig(
                    ...
                    issue_discovery_share=_coerce_share(
                        ...
                        metadata.get('issue_discovery_share', _DEFAULT_ISSUE_DISCOVERY_SHARE),
                    ),
