...
class TestScoreCommand:
    ...
    def test_profile_rejects_missing_parent_directory(self, runner, tmp_path):
        bad_path = tmp_path / 'missing' / 'run.prof'
        result = runner.invoke(
            cli,
            ['miner', 'score', '--profile', str(bad_path), '--pat', 'ghp_dummy'],
            env={},
        )
        assert result.exit_code == 1
        assert 'Profile output directory does not exist' in result.output

    def test_profile_rejects_directory_path_in_json_mode(self, runner, tmp_path):
        result = runner.invoke(
            cli,
            ['miner', 'score', '--json', '--profile', str(tmp_path), '--pat', 'ghp_dummy'],
            env={},
        )
        assert result.exit_code == 1
        payload = json.loads(result.output)
        assert payload['success'] is False
        assert 'Profile output path points to a directory' in payload['error']['message']
