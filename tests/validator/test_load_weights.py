...
class TestLoadMasterRepositories:
    ...
    def test_live_master_repositories_use_emission_share_schema(self, repo_name, metadata):
        assert 'emission_share' in metadata, f'{repo_name} must use emission_share'
...
class TestRepositoryConfigMirrorScoringFields:
    ...
    def test_loader_parses_mirror_scoring_fields(self, tmp_path, monkeypatch):
        ...
        assert repos['foo/fixed'].issue_discovery_share == pytest.approx(0.25)
    ...
    def test_live_mirror_scoring_fields_have_valid_shape(self):
        ...
        for repo_name, config in repos.items():
            ...
            assert 0.0 <= config.emission_share <= 1.0, f'{repo_name} emission_share must be within [0.0, 1.0]'
...
class TestRepositoryEmissionShares:
    """Tests for bounded per-repository emission shares."""

    @pytest.mark.parametrize('emission_share', [0.0, 0.0349, 0.5, 1.0])
    def test_preserves_full_precision(self, emission_share):
        config = RepositoryConfig(emission_share=emission_share)
        assert config.emission_share == pytest.approx(emission_share)

    def test_live_master_repo_emission_total_is_bounded(self):
        repos = load_master_repo_weights()
        assert sum(config.emission_share for config in repos.values()) <= 1.0

    def test_oc1_opts_out_of_issue_discovery_slice(self):
        repos = load_master_repo_weights()
    ...
    @pytest.mark.parametrize(
        'metadata',
        [
            {'foo/bad': {'emission_share': -0.1}},
            {'foo/bad': {'emission_share': 1.1}},
            {'foo/bad': {'emission_share': 0.5, 'issue_discovery_share': -0.1}},
            {'foo/bad': {'emission_share': 0.5, 'issue_discovery_share': 1.1}},
            {'foo/bad': {'label_multipliers': {'feature': 1.5}}},
        ],
    )
    def test_loader_rejects_invalid_repository_share_entries(self, tmp_path, monkeypatch, metadata):
        from gittensor.validator.utils import load_weights as lw

        (tmp_path / 'master_repositories.json').write_text(json.dumps(metadata))
    ...
    def test_loader_accepts_registry_sum_below_one(self, tmp_path, monkeypatch):
        from gittensor.validator.utils import load_weights as lw
