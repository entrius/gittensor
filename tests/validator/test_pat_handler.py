...
def _transient_github_identity() -> GitHubIdentityResult:
    return GitHubIdentityResult(None, GitHubIdentityStatus.TRANSIENT_FAILURE)


def _github_validation(
    github_id: str | None,
    error: str | None = None,
    transient_failure: bool = False,
) -> GitHubCredentialValidation:
    return GitHubCredentialValidation(github_id, error, transient_failure)
...
class TestHandlePatBroadcast:
    ...
    @patch(
        ...
        return_value=_github_validation('github_42'),
    )
    ...
    @patch(
        'gittensor.validator.pat_handler.validate_github_credentials_result',
        return_value=_github_validation(None, 'PAT invalid'),
    )
    ...
    @patch('gittensor.validator.utils.github_validation.get_github_identity', return_value=_transient_github_identity())
    def test_transient_github_failure_rejected_with_retry_message(self, mock_identity, mock_validator):
    ...
    @patch(
        'gittensor.validator.pat_handler.validate_github_credentials_result',
        return_value=_github_validation('github_99'),
    )
...
class TestHandlePatCheck:
    ...
    @patch(
        'gittensor.validator.pat_handler.validate_github_credentials_result',
        return_value=_github_validation(None, 'PAT expired'),
    )
    ...
    @patch('gittensor.validator.utils.github_validation.get_github_identity', return_value=_transient_github_identity())
    def test_transient_github_failure_keeps_stored_pat_inconclusive(self, mock_identity, mock_validator):
