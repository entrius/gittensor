...
class TestCalculatePioneerRanks:
    ...
    def test_single_miner_is_pioneer(self, pr_factory, bronze_config):
        now = datetime.now(timezone.utc)
        pr = pr_factory.create(state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1, merged_at=now)
    ...
    def test_earliest_merge_gets_rank_one(self, pr_factory, bronze_config):
        now = datetime.now(timezone.utc)
        pr1 = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1, merged_at=now - timedelta(days=10),
        )
        pr2 = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=2, merged_at=now - timedelta(days=5),
        )
        evals = {
    ...
    def test_same_timestamp_breaks_tie_by_pr_number(self, pr_factory, bronze_config):
        """Same-second merges (e.g. merge bot queue) produce a single winner via PR number."""
        now = datetime.now(timezone.utc)
        pr_high = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1, number=20, merged_at=now,
        )
        pr_low = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=2, number=3, merged_at=now,
        )
        evals = {
            1: MinerEvaluation(uid=1, hotkey='hk1', merged_pull_requests=[pr_high]),
            2: MinerEvaluation(uid=2, hotkey='hk2', merged_pull_requests=[pr_low]),
        }
        ranks, _ = calculate_pioneer_ranks(evals)
        assert ranks['org/repo-a'][2] == 1  # lower PR number wins
        assert ranks['org/repo-a'][1] == 2

    def test_repos_ranked_independently(self, pr_factory, bronze_config):
        now = datetime.now(timezone.utc)
        pr1a = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1, merged_at=now - timedelta(days=10),
        )
        pr2a = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=2, merged_at=now - timedelta(days=5),
        )
        pr1b = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-b', uid=1, merged_at=now - timedelta(days=3),
        )
        pr2b = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-b', uid=2, merged_at=now - timedelta(days=8),
        )
        evals = {
    ...
    def test_multiple_prs_same_repo_uses_earliest(self, pr_factory, bronze_config):
        now = datetime.now(timezone.utc)
        early = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1, merged_at=now - timedelta(days=20),
        )
        late = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1, merged_at=now - timedelta(days=5),
        )
...
class TestQualityGate:
    ...
    def test_low_token_score_excluded(self, pr_factory, bronze_config):
        now = datetime.now(timezone.utc)
        low = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1,
            merged_at=now - timedelta(days=10), token_score=MIN_TOKEN_SCORE_FOR_BASE_SCORE - 1,
        )
        good = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=2, merged_at=now - timedelta(days=5),
        )
        evals = {
    ...
    def test_no_tier_config_excluded(self, pr_factory, bronze_config):
        now = datetime.now(timezone.utc)
        untracked = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1, merged_at=now - timedelta(days=10),
        )
...
class TestFinalization:
    ...
    def test_pioneer_gets_bonus_follower_gets_nothing(self, pr_factory, bronze_config):
        ...
        pr2 = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=2,
    ...
    def test_multi_repo_damping_applied(self, pr_factory, bronze_config):
        """Pioneering 4 repos yields damped bonus per repo."""
        now = datetime.now(timezone.utc)
        prs = []
        for i in range(4):
            pr = pr_factory.create(
                state=PRState.MERGED, tier=bronze_config, repo=f'org/repo-{i}', uid=1,
    ...
    def test_low_quality_snipe_blocked(self, pr_factory, bronze_config):
        """Low-quality early merge gets 1.0, quality follower becomes pioneer."""
        now = datetime.now(timezone.utc)
        snipe = pr_factory.create(
            state=PRState.MERGED, tier=bronze_config, repo='org/repo-a', uid=1,
            merged_at=now - timedelta(days=10), token_score=MIN_TOKEN_SCORE_FOR_BASE_SCORE - 1,
...
class TestPioneerIncentiveEvidence:
    ...
    def test_exploration_beats_pile_on(self, pr_factory, bronze_config):
        ...
        for uid in range(1, 6):
            pr = pr_factory.create(
                state=PRState.MERGED, tier=bronze_config, repo='org/saturated', uid=uid,
        ...
        for uid in range(1, 6):
            pr = pr_factory.create(
                state=PRState.MERGED, tier=bronze_config, repo=f'org/new-{uid}', uid=uid,
                merged_at=now - timedelta(days=uid), earned_score=0.0, collateral_score=0.0,
            )
            pr.base_score = 30.0
